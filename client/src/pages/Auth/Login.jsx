...
import {
  ...
  FaLock, FaEnvelope, FaChevronRight,
} from "react-icons/fa";
  ...
  <div className="mb-5">
    ...
    <div className="relative">
      ...
      <input
        ...
        className={`block w-full pl-11 pr-4 py-4 rounded-2xl bg-[#F0F8FF] text-[#274A63] placeholder-[#4682B4]/40 border-none outline-none transition-shadow duration-200 ${INSET} focus:ring-2 focus:ring-[#87CEEB] focus:ring-offset-2 focus:ring-offset-[#F0F8FF]`}
      />
    </div>
  </div>
);
  ...
  return (
    <div className="min-h-screen w-full flex overflow-x-hidden bg-gradient-to-br from-[#F7FBFF] to-[#DCEBF7]">
      ...
      >
        <div>
          ...
          <p className="text-[#4A6B84] text-lg max-w-sm leading-relaxed">
            Every expense, categorized and visualized : so budgeting stops
              ...
              >
                <div className="grid grid-cols-2 gap-4 mb-8">
                  {premiumFeatures.map((feature, index) => (
                    <div
                      key={index}
                      className={`bg-[#F0F8FF] p-4 rounded-2xl ${RAISED_SM}`}
                    >
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8">
        {/* Brand line for small screens, where the left panel is hidden */}
        ...
        <motion.div
          ...
          className={`max-w-md w-full bg-[#F0F8FF] rounded-[32px] p-10 sm:p-12 ${RAISED}`}
        >
          <div className="text-center">
            <div className={`mx-auto w-16 h-16 rounded-2xl bg-[#F0F8FF] flex items-center justify-center text-[#4682B4] mb-6 ${RAISED_SM}`}>
              <FaLock size={22} />
            </div>
            <h2 className="text-2xl font-bold text-[#22415A] mb-2">Secure login</h2>
            <p className="text-[#4A6B84] text-sm">
              Enter your details to open your dashboard
            </p>

            <div className={`mt-5 p-4 rounded-2xl bg-[#F0F8FF] text-sm text-[#2F5F82] text-left ${INSET}`}>
              <strong className="text-[#22415A]">Note:</strong> if login fails, wait
              a moment and try again — the server can take a little time to wake up.
            </div>
          </div>

          {error && (
            <div className="mt-5 rounded-2xl bg-red-50 p-4 border border-red-200">
          ...
          <form className="mt-7" onSubmit={handleLogin}>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className={`group relative w-full flex items-center justify-center py-4 px-4 mt-2 text-base font-semibold rounded-2xl text-white bg-[#4682B4] hover:bg-[#3E76A4] disabled:opacity-80 transition-all duration-150 ${RAISED_SM} ${PRESSED}`}
            >
              {isLoading ? (
                <>
          ...
          <div className="pt-6 mt-6 border-t border-[#4682B4]/10 text-center">
            <p className="text-[#4A6B84] text-sm">
              New to OutlayLY?{" "}
              <Link to="/signup" className="text-[#4682B4] hover:text-[#2F5F82] font-medium transition-colors">
                Sign up here
              </Link>
            </p>
          </div>
